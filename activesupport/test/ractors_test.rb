...
class RactorsTest < ActiveSupport::TestCase
  ...
  def test_fallback_when_ractor_is_unavailable
    Object.send(:remove_const, :Ractor) if defined?(Ractor)
    ActiveSupport.send(:remove_const, :Ractors)
    load File.expand_path("../lib/active_support/ractors.rb", __dir__)

    assert ActiveSupport::Ractors.main?
    assert_equal 2, ActiveSupport::Ractors.shareable_lambda { 1 + 1 }.call

    ActiveSupport::Ractors[:key] = "value"
    assert_equal "value", ActiveSupport::Ractors[:key]
    assert_equal "stored", ActiveSupport::Ractors.store_if_absent(:stored) { "stored" }
  end
